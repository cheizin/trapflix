...
if($_SERVER['REQUEST_METHOD'] == 'POST')
{
 ...
 if(isset($_POST['mail-validation']))
{
  ...
  else
  {
    ...
    $sql_insert= mysqli_query($dbc,"INSERT INTO password_resets
          (token, email)
            VALUES ('".$token."','".$email."')") or die (mysqli_error($dbc));


    //log the action
    $action_reference = "Added Personal Details " . $fName;
...
else
{
  ...
  exit("failed");
    ...
    $sql_insert= mysqli_query($dbc,"INSERT INTO job_titles
          (title_name, recorded_by)
            VALUES ('".$job_title."','".$recorded_by."')") or die (mysqli_error($dbc));
