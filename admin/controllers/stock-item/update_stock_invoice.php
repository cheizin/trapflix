...
if($_SERVER['REQUEST_METHOD'] == 'POST')
{
  ...
  $total = mysqli_real_escape_string($dbc,strip_tags($_POST['total']));
    ...
    if(!empty($_FILES["file"]["name"])){
      ...
      $sql_modify_stock = mysqli_query($dbc,
                                  "UPDATE single_product SET
                                      ...
                                      VALUES ('".$reference_no_stock."','".$invoice_received_id."','".$supplier_name."', '".$unit_price."', '".$qtt."','".$stock_order_level."', '".$total."',
                                        '".$uploadedFile."','".$additional_file."', '".$additional_file2."','".$payment_type."','".$date_recorded."','".$recorded_by."')") or die (mysqli_error($dbc));

    //log the action
    $action_reference = "Updated Purchased Stock for Item " .$stock_name;
