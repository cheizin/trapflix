...
if($_SERVER['REQUEST_METHOD'] == 'POST')
{
  ...
  //  $comments = mysqli_real_escape_string($dbc,strip_tags($_POST['comments']));
    ...
    $sql_insert = " UPDATE customer_end_delivery SET status='".$status."' WHERE id ='".$risk_reference."' ";
    $query_insert = mysqli_query($dbc,$sql_insert);

    //log the action
    $action_reference = $status. " End Product Delivery ".$risk_reference;
    $action_name = "Delivery Approvals";
    if($status == 'approved')
    {
