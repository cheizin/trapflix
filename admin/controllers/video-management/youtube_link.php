...
require_once('../middleware/UserIsAuthenticated.php');
  ...
  if(isset($_POST['add-youtube-list']))
  {
    ...
    //start get youtube video id
    $urlParts   = explode('=', $videoname);
    $vidid      = explode( '&', str_replace('https://www.youtube.com/watch?v=', '', end($urlParts) ) );
    $videoname = $vidid[0] ;
    
    //end get youtube video id
    ...
    //end file upload
     ...
     $sql_statement = mysqli_query($dbc,"INSERT INTO videos
                                                (email,textname,video_description, title, videoname, token,thumbnail)
                                        VALUES
                                                ('".$email."','".$textname."', '".$video_description."', '".$title."', '".$videoname."', '".$token."','".$thumbnail."')
