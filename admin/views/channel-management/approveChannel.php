...
<nav aria-label="breadcrumb">
     <ol class="breadcrumb">
       <li class="breadcrumb-item active" aria-current="page">Approve Channel Management</li>
     </ol>
</nav>
    ...
    <div class="card card-primary card-outline">
      ...
      <div class="card-body table-responsive">
        ...
        {?>
         ...
         <?php
         $no = 1;
          $sql = mysqli_query($dbc,"SELECT * FROM main_categories WHERE category_name !='Popular On Trapflix' ORDER BY id DESC");
...
<?php
 ...
 $milestone_name = mysqli_query($dbc,"SELECT * FROM main_categories WHERE id ='".$row['id']."' && approved ='yes'") or die(mysqli_error($dbc));
 if(mysqli_num_rows($milestone_name) > 0){

?>

<a href="#" class="btn btn-link" data-toggle="tooltip" title="Click here to remove it from Channel List" onclick="disapproveChannel('<?php echo $row['id'];?>');">
...
{
  ?>
  <a href="#" class="btn btn-link" data-toggle="tooltip" title="Click here to Approve the Channel" onclick="approveChannel('<?php echo $row['id'];?>');">
