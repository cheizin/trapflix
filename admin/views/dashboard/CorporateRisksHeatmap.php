<div class="row">
  <div class="col-md-8">
    <div class="card">
      <div class="card-header bg-light">Top 20 Corporate Risks</div>
      <div class="card-body">
        <table class="table table-bordered table-responsive">
           <?php
              $sql_query = mysqli_query($dbc,"SELECT * FROM update_risk_status WHERE risk_opportunity='risk' && changed='no'  && period_from='".$select_period."' && quarter='".$select_quarter."' && current_overall_score >='20' ORDER BY current_overall_score DESC LIMIT 20");
              $sql_query_risk_position = mysqli_fetch_array($sql_query);
              ?>
           <tbody>
              <tr>
                 <td rowspan="5" class="impact_rotate">Impact</td>
                 <td>Catastrophic <small class="text-primary">5</small></td>
                 <td class="medium" style="background-color: #FFFF00;"  title="OVERALL SCORE: 5">
                    <?php
                       $sql = mysqli_query($dbc,"SELECT * FROM update_risk_status
                                                  ...
                                                  && risk_opportunity='risk'  && period_from='".$select_period."' && quarter='".$select_quarter."' && current_overall_score >='20' && risk_status='open' ORDER BY current_overall_score DESC LIMIT 20 ");
        ...
        <table class="table table-bordered">
           ...
           <tr>
              <td class="text-danger">Sorry, no records have been found for
                 this quarter (<span class="text-info"><?php echo $select_quarter?></span>) and period (<span class="text-info"><?php echo $select_period?> at the Corporate Level</span>)
              </td>
           </tr>
        </table>
        <?php
