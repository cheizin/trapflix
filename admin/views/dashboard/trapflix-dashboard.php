<?php
  ...
  $users_sql = mysqli_query($dbc,"SELECT * FROM users ");
    ...
    // active users
    $users_sql2 = mysqli_query($dbc,"SELECT * FROM users WHERE user_status ='Active'");
      $total_users2 = mysqli_num_rows($users_sql2);

    // deactivated users

    $users_sql3 = mysqli_query($dbc,"SELECT * FROM users WHERE user_status ='deactivated'");
      $total_users3 = mysqli_num_rows($users_sql3);


    $videos_sql = mysqli_query($dbc,"SELECT * FROM videos");


      $total_videos = mysqli_num_rows($videos_sql);

      //Youtube videos
      $youtube_sql = mysqli_query($dbc,"SELECT * FROM videos WHERE youtube_vid  ='youtube'");


        $total_youtube = mysqli_num_rows($youtube_sql);

        // server videos
        $server_sql = mysqli_query($dbc,"SELECT * FROM videos WHERE youtube_vid  ='server'");


          $total_server = mysqli_num_rows($server_sql);
  // channel videos
  $total_channels = mysqli_num_rows(mysqli_query($dbc,"SELECT * FROM main_categories"));

    $server_channels = mysqli_num_rows(mysqli_query($dbc,"SELECT * FROM main_categories WHERE approved ='yes'"));

      $youtube_channels = mysqli_num_rows(mysqli_query($dbc,"SELECT * FROM main_categories WHERE approved ='no'"));
...
$get_details = 'Loss';
  ...
  $resource_utilization_sql = mysqli_query($dbc,"SELECT email, count(*) AS videoz FROM videos GROUP BY email");
...
<nav aria-label="breadcrumb">
     <ol class="breadcrumb">
       <li class="breadcrumb-item active" aria-current="page"><strong>TRAPFLIX Management Dashboard</strong></li>
     </ol>
</nav>
    ...
    <div class="row">
      <div class="col-12 col-sm-6 col-md-3">
        <div class="info-box" title="Total Stocks Available">
          ...
          <div class="info-box-content hvr-overline-from-center">
            <span class="info-box-text">  <strong><?php echo $total_users;?></strong> Users</span>
            <span class="info-box-number">
              <small>(<span class="text-success"><?php echo $total_users2;?> Active Users</span>)</small>  </br>
              <small>(<span class="text-success"><?php echo $total_users3;?> Deactivated Users</span>)</small>
            </span>
          ...
          <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
              <div class="modal-header bg-light">
                <h5 class="modal-title"><?php echo $total_users;?> Trapflix Register Details </h5>
      ...
      <div class="col-12 col-sm-6 col-md-3">
        <div class="info-box mb-3" title="Total Resources for Active Projects">
          ...
          <div class="info-box-content hvr-overline-from-center">
            <span class="info-box-text">  <strong>  Total Revenue </span>
        ...
        <div class="modal fade" id="total-profit-modal" tabindex="-1" role="dialog">
          <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
              <div class="modal-header bg-light">
                <h5 class="modal-title">Total Revenues</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">&times;</span>
                </button>
              </div>
        ...
        <div class="card">
          ...
          <div class="card-body">
            <div class="row">
              <div class="col-md-6">
                <p class="text-center">
                  <strong>Videos Uploaded Per User</strong>
                </p>
                <div class="chart">
                  ...
                  <table class="table table-hover table-striped table-bordered" id="dashboard-overdue-tasks-table">
                     ...
                     <tbody>
                       ...
                       <?php
                          ...
                          $sql234= mysqli_query($dbc,"SELECT email, count(*) AS vidz FROM videos GROUP BY email");
